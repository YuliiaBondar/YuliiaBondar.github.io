window.funnelOptions = {
  ...
  pathPrefix: "../../",
  funnel_uri: "/elcq_7229_egi",
  facebookPixelId: "780104776000487",
  noRedirectBrand: true,
};
...
(function () {
  ...
  leadFormFunc.prototype.getToken = function () {
    ...
    $.ajax({
      url: "https://api.tlpdaisy.com/api/page-properties",
      type: "POST",
      dataType: "json",
      data: { pathName: "/api_for_aws_do_not_delete" },
      context: this,
  ...
  leadFormFunc.prototype.createUser = function (csrf) {
    ...
    var dataObj = {
      ...
      funnel_uri: globalOptions.funnel_uri,
      funnel_atts: globalOptions.funnel_atts,
    ...
    var marketingParams = window.location.search;
    ...
    function isNumeric(n) {
      return (
        typeof n == "number" ||
        (typeof n == "string" && n.match(/^[0-9]+$/) != null)
      );
    }

    let urlParametersObj = getJsonFromUrl(window.location.search);
    for (let i = 0; i < Object.keys(urlParametersObj).length; i++) {
      let parameterKey = Object.keys(urlParametersObj)[i];
      let parameterValue = Object.values(urlParametersObj)[i];
      if (parameterKey === "link" && isNumeric(parameterValue)) {
        marketingParams += "&campaign_id=" + parameterValue;
      } else if (parameterKey === "coc" && isNumeric(parameterValue)) {
        marketingParams += "&campaign_id=" + parameterValue;
      } else if (parameterKey === "campaignid" && isNumeric(parameterValue)) {
        marketingParams += "&campaign_id=" + parameterValue;
      } else if (parameterKey === "campaign_id" && isNumeric(parameterValue)) {
        marketingParams += "&campaign_id=" + parameterValue;
      } else if (parameterKey === "campaign" && isNumeric(parameterValue)) {
        marketingParams += "&campaign_id=" + parameterValue;
      }
    }
    ...
    $.ajax({
      type: "POST",
      url: "https://api.tlpdaisy.com/api/leads",
...
$(document).ready(function () {
  ...
  $.ajax({
    url: "https://api.tlpdaisy.com/api/visitor-country",
    type: "GET",
    dataType: "json",
    success: function (result) {
  ...
  $.ajax({
    url:
      "https://api.tlpdaisy.com/api/brands/" +
...
document.addEventListener("DOMContentLoaded", function (event) {
  $(document).on(
    "click",
    ".nrp__close, .nrp__btn, .nrp__overlay",
    function (event) {
      $(".nrp").hide();
    }
  );
